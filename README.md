...
There are Solidity wrappers that are created by `abigen` from the `go-ethereum` package.  To generate, the `.abi`/`.bin` files are required from the step above. These wil go into `pkg/generated/contract` directory and are generated by running:
